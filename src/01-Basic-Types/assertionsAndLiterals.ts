...
//尝试欺骗
let value6 : unknown = 123;
console.log((value6 as string).length);
...
let user_1: "admin" | "user";
let user_2: "admin" | "user";
let user_3: "admin" | "user";
// ... 还要写7次，代码冗余且容易写错
//  写法2：类型别名 → 只定义1次，到处复用
// type Role = "admin" | "user";
// let user1: Role;
...
const roles2 = ["admin", "user"] as const;  //悬停显示是roles2 : ["admin", "user"]

//常见高级写法
console.log("===高级写法===")
const roles = ["admin", "user"] as const;
// 1. typeof roles → 获取 roles 的类型：readonly ["admin", "user"]
// 2. [number] → 取出这个元组中「所有数字索引对应的元素类型」
type Roles = typeof roles[number];
let role_exam : Roles;
role_exam = "admin";
console.log(typeof roles);  //输出object
