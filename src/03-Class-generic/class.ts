...
//定义Person类
class PersonClass {
    //公共属性： 默认也是public，显式声明更规范
    public name : string;

    //只读属性： 只能在构造函数中声明，后续不可以更改
    readonly age : number;

    //私有属性：只能在类内部访问，外部无法直接获取/修改
    private id : number;
    ...
    introduce(this : PersonClass) : string {
        ...
        return `大家好，我叫${this.name}，今年${this.age}岁，我的内部编号是${this.id}。`;
    }
...
// ------------------- 测试代码 -------------------
// 创建 Person 实例
const personE1 = new PersonClass("张三", 25);
const personE2 = new PersonClass("李四", 30);
