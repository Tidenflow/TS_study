...
function testGenericFunctions() {
  ...
  const str = identity<string>("Hello TypeScript"); // 显式指定 T 为 string
  const num = identity(123); // 隐式推导 T 为 number
  const bool = identity(true); // 隐式推导 T 为 boolean
  ...
  // 合法："name" 是 user 的键
  const userName = pickFirst(user, "name"); 
  console.log(userName); // 张三
  // 合法："age" 是 user 的键
  const userAge = pickFirst(user, "age"); 
  ...
  console.log("\n=== 测试 createPair 函数 ===");
  const pair1 = createPair("name", "李四"); // [string, string]
  const pair2 = createPair(1, "apple"); // [number, string]
  const pair3 = createPair(true, { id: 1 }); // [boolean, { id: number }]
