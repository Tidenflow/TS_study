...
function buildName(firstName, lastName, age = 18, ...hobbies) {
    let fullName = firstName;
    if (lastName)
        fullName += `${lastName}`;
    fullName += ` (年龄: ${age})`;
    if (hobbies.length > 0)
        fullName += `(爱好: ${hobbies.join(", ")})`;
    return fullName;
}
// 调用示例
